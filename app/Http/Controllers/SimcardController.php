...
class SimcardController extends Controller
{
   public function buscarSimcard(Request $request)
    ...
    public function buscarPaquete(Request $request)
    {
        if($request->ajax()){
            $colors = [];
            if($request['paquete'] != null){
                $simcard = \DB::table('simcards')->where('paquete',$request['paquete'])->get();
                $months = \DB::table('simcards')->select(\DB::raw('DATEDIFF(CURDATE(),fecha_vencimiento) as month'))->where('paquete',$request['paquete'])->get(); 
                $i = 0;
